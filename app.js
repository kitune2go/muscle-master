...
const defaultState={userName:'',trainerName:'',days:{},totalSets:0,stats:{strength:0,core:0,mobility:0,endurance:0},sound:true,lastShownLevel:1};
...
function getStreak(){let streak=0,cursor=new Date();while(true){const key=todayKey(cursor);if(completedSetsForKey(key)===0)break;streak++;cursor.setDate(cursor.getDate()-1);}return streak;}
...
function showToast(text){const t=document.querySelector('#toast');t.textContent=text;t.classList.add('show');clearTimeout(showToast.timer);showToast.timer=setTimeout(()=>t.classList.remove('show'),1500);}
...
function render(){renderPlanLabels();renderHome();renderQuest();renderStatus();renderHistory();document.querySelector('#soundButton').classList.toggle('muted-sound',!state.sound);document.querySelector('#soundButton').textContent=state.sound?'♪':'×';saveState();bindDynamicLinks();}
function toggleSet(ex,index){const day=ensureDay(),previousLevel=level(),wasDone=day[ex.id][index];day[ex.id][index]=!wasDone;state.totalSets=Math.max(0,state.totalSets+(wasDone?-1:1));addStats(ex,wasDone?-1:1);saveState();render();if(wasDone){playUndo();showToast('セットを取り消しました');}else{playComplete();pulseTrainer();showToast(`${ex.name} SET ${index+1} 完了！ +10 XP`);checkLevelUp(previousLevel);if(getProgress()===100){confetti();setTimeout(()=>playLevelUp(),120);}}}
...
document.querySelector('#settingsButton').addEventListener('click',()=>{document.querySelector('#userNameInput').value=state.userName||'';document.querySelector('#trainerNameInput').value=state.trainerName||'';settingsDialog.showModal();});
document.querySelector('#settingsForm').addEventListener('submit',event=>{event.preventDefault();state.userName=document.querySelector('#userNameInput').value.trim();state.trainerName=document.querySelector('#trainerNameInput').value.trim();saveState();settingsDialog.close();render();});
document.querySelector('#soundButton').addEventListener('click',()=>{state.sound=!state.sound;saveState();render();if(state.sound){tone(660,.08,'triangle');showToast('効果音 ON');}else showToast('効果音 OFF');});
...
ensureDay();if(!state.lastShownLevel)state.lastShownLevel=level();render();showView(['home','quest','status','log'].includes(location.hash.slice(1))?location.hash.slice(1):'home');if(!state.userName&&!state.trainerName)setTimeout(()=>settingsDialog.showModal(),300);
