...
- JSONクエスト基盤
  ...
  - ボーナスXP、受取台帳、表示待ち演出を同一保存へ確定
  - 画像・音声URL・トレーナー台詞の差し替え口を定義
- ミッション・報酬画面
  - 「今日のメニュー」と同一クエスト画面内で切替
  - デイリー／ウィークリーの進捗、達成、受取状態を表示
  - 達成済みカードからボーナスXPを1回だけ受取
  - 再読み込みやセット取消後も受取済み報酬を維持
  - 定義パック取得失敗時も通常のトレーニング記録を継続
- 報酬演出キュー
  - 受取確定後にトレーナー台詞、報酬XP、演出等級を表示
  - 演出中断後は再読み込み時に未確認分を再表示
  - 報酬でレベル閾値を越えた場合はレベルアップ演出へ順番に接続
  - 効果音OFFと `prefers-reduced-motion` を尊重
- セット完了ごとの +10 XP
- レベルアップ演出 / 紙吹雪
...
- `quest-core.js` — クエスト期間、進捗評価、初回達成、報酬の一度だけの付与、演出キューの共通ロジック
...
クエスト基盤は現在、進捗評価、達成記録、ミッション表示、ボーナスXPの一度だけの付与、保存可能な達成演出キューまでを担当します。
